...
class WaveOrchestrator:
    ...
    async def run(self, goal: str, domain: str = "general", team: str = "default", task_run_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        # Redactions are applied in-place so the swarm never sees raw PII/secrets.
        _ingress_verdicts: list = []
        try:
            from src.swarm.guardrails import get_guardrail_layer, GuardrailError as _GuardrailError
            _gl = get_guardrail_layer()
            goal, _ingress_verdicts = await asyncio.to_thread(_gl.run_ingress, goal)
        except Exception as _gr_exc:
            # GuardrailError subclasses Exception; re-raise it so callers see the block.
            from src.swarm.guardrails import GuardrailError as _GuardrailError
            if isinstance(_gr_exc, _GuardrailError):
                logger.warning("[Guardrail:INGRESS:ABORT] %s", _gr_exc)
                raise
            # Non-guardrail errors (e.g. import failure): fail-loud — block the run.
            logger.error("[Guardrail:INGRESS:ERROR] %s — blocking run", _gr_exc)
            raise RuntimeError(f"Guardrail ingress error (blocking by default): {_gr_exc}") from _gr_exc
        ...
        # ── Egress guardrails: output safety + code shield ────────────────────
        # Applied to the full synthesis before it is returned. Blocks are recorded
        # and appended to SENTINEL_ALERTS so they are visible in the governance
        # dashboard. The synthesis is still returned but flagged so the caller (the
        # task runner / API layer) can decide whether to surface it or suppress it.
        _egress_verdicts: list = []
        try:
            from src.swarm.guardrails import get_guardrail_layer
            _gl = get_guardrail_layer()
            _egress_verdicts = await asyncio.to_thread(_gl.run_egress, synthesis_text)
            _eg_flags = _gl.egress_flags(_egress_verdicts)
            if _eg_flags:
                final["GUARDRAIL_FLAGS"] = [
                    {
                        "classifier": v.classifier,
                        "action": v.action.value if hasattr(v.action, "value") else str(v.action),
                        "confidence": v.confidence,
                        "reason": v.reason[:300],
                        "patterns": v.patterns_matched[:10],
                    }
                    for v in _eg_flags
                ]
            if _gl.egress_blocked(_egress_verdicts):
                final["EGRESS_BLOCKED"] = True
                logger.warning(
                    "[Guardrail:EGRESS:BLOCK] Synthesis blocked by guardrail classifiers."
                )
            # Feed guardrail verdicts into the governance compliance / HR signals.
            cs_delta = _gl.verdicts_to_compliance_delta(_egress_verdicts + _ingress_verdicts)
            hr_delta = _gl.verdicts_to_hr_delta(_egress_verdicts + _ingress_verdicts)
            if cs_delta != 0.0 or hr_delta != 0.0:
                final["GUARDRAIL_GOVERNANCE_IMPACT"] = {
                    "cs_delta": round(cs_delta, 1),
                    "hr_delta": round(hr_delta, 3),
                }
                logger.info(
                    "[Guardrail:GOVERNANCE] cs_delta=%.1f hr_delta=%.3f",
                    cs_delta, hr_delta,
                )
        except Exception as _eg_exc:
            # Egress errors are fail-loud: flag the output.
            logger.error("[Guardrail:EGRESS:ERROR] %s — flagging output", _eg_exc)
            final["GUARDRAIL_FLAGS"] = [{"classifier": "error", "action": "flag",
                                         "confidence": 1.0, "reason": str(_eg_exc)[:200],
                                         "patterns": []}]
