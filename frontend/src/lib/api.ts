...
export interface GovernanceProposal { id: number; proposal_id: string; title: string; track: string; origin: string; status: string; proposed_action: string; quorum_threshold: number; yes: number; no: number; abstain: number; yes_ratio: number; total_effective: number; vote_count: number; participation: ParticipationStatus; no_op_enactment?: boolean; created_at: string }
export interface RoutingStats { total_evaluations: number; zero_token_hits: number; zero_token_misses: number; threshold: number; embedding_backend: string; last_hit_capability: string | null; last_hit_score: number; hit_rate: number; hit_rate_pct: number; last_reset_at: string }
export interface DriftEntry { capability: string; reason: string; severity: string }
...
export interface Subscription { id: number; name: string; domain: string; login_url: string; account_url: string; login_username: string; has_credential: boolean; status: string; est_cost: string; cost_is_estimate: boolean; currency: string; source: string; unknown: boolean; needs_details: boolean; notes: string; last_checked_at: string | null; created_at: string }
